    ...
    <div className="flex min-h-screen items-center justify-center bg-surface-2">
      <div className="w-full max-w-sm rounded-xl bg-white p-8 shadow-lg">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-ink-3">重設密碼</p>
        </div>

        {step === "key" && (
          <form onSubmit={verifyKey} className="space-y-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-ink-2">重設金鑰</label>
              <input
                type="text"
                value={key}
              ...
              />
              <p className="mt-1 text-xs text-ink-3">請輸入管理員提供的密碼重設金鑰</p>
            </div>
        ...
        {step === "form" && (
          <form onSubmit={handleReset} className="space-y-4">
            <div className="rounded-lg bg-accent-soft p-3 text-sm">
              ...
              <p className="text-xs text-accent">重設密碼</p>
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium text-ink-2">新密碼</label>
              <input
                type="password"
                value={password}
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-ink-2">確認新密碼</label>
              <input
                type="password"
                value={password2}
