...
const navItems = [
  ...
  { href: "/cases", label: "個案管理", icon: "👤", roles: ["admin", "accountant", "staff", "therapist"] },
  { href: "/calendar", label: "預約日曆", icon: "📅" },
  { href: "/ledger", label: "日結帳冊", icon: "📒", roles: ["admin", "accountant", "staff", "therapist"] },
  { href: "/claims", label: "核銷案管理", icon: "💰", roles: ["admin", "accountant", "staff", "therapist"] },
  ...
  return (
    <aside className="flex h-screen w-60 flex-col border-r border-gray-200 bg-white">
      ...
      <nav className="flex-1 overflow-y-auto px-2 py-4">
        {navItems
          .filter((item) => !("roles" in item) || (item.roles as readonly string[]).includes(userRole))
          .map((item) => {
